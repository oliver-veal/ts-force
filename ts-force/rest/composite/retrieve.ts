import { CompositeBatch, CompositeBatchResult, isCompositeBatchFailResult, CompositeBatchFailResult, Composite } from './index.ts';
import { SObjectStatic, FieldResolver, SOQLQueryParams, QueryResponse, buildQuery, SObject, StandardRestError, Rest } from '../../index.ts';
...
export const queryAllComposite = async (query: string, opts: { restInstance?: Rest, allRows?: boolean, nextUrl?: string }): Promise<SObject[]> => {
  ...
  for (let i = 0; i < response.compositeResponse.length; i++) { // subResponse of response.compositeResponse) {
    ...
    if (subResponse.httpStatusCode === 400 && Array.isArray(subResponse.body) && subResponse.body.length && subResponse.body[0].message.indexOf('Invalid reference specified') >= 0) {
      break;
