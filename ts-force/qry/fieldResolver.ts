import {
  ParentReferencePropNames,
  QueryField,
  RelationPropNames,
} from "../types.ts";

import { SFieldProperties } from "../index.ts";
import {
  composeSOQLQuery,
  SObjectStatic,
  SOQLQueryParams,
...
export class FieldResolver<T> {
  ...
  public select(
    ...args: Array<QueryField<T> | FunctionField<T>> | [
      Array<QueryField<T> | FunctionField<T>>,
    ]
  ): string | string[] {
      ...
      if (Array.isArray(field)) {
        field.forEach((arrField) => {
          if (typeof arrField === "string") {
            ...
            fieldArr.push(
              renderComplexTypeText(
                // @ts-ignore
                this._obj.FIELDS[arrField.field as string].apiName,
                arrField.fn,
                arrField.alias,
              ),
            );
        ...
        fieldArr.push(
          renderComplexTypeText(
            // @ts-ignore
            this._obj.FIELDS[field.field as string].apiName,
            field.fn,
            field.alias,
          ),
        );
   ...
   */
  public subQuery<
    K extends RelationPropNames<Required<T>>,
    T2 extends Required<T>[K] extends (infer U)[] ? U : never,
  >(
    relationship: K,
    func: (fields: FieldResolver<T2>) => SOQLQueryParams,
  ): string {
    // @ts-ignore
    let relationMeta = this._obj.FIELDS[relationship as string];
    let fields = new FieldResolver(
      relationMeta.reference() as any as SObjectStatic<T2>,
    );
    let subQuery = func(fields);
    return `(${
      composeSOQLQuery({ ...subQuery, ...{ from: relationMeta.apiName } })
    })`;
  }
}

export function renderComplexTypeText(
  field: string,
  func: string,
  alias?: string,
) {
  const ret = `${func}(${field})${alias ? " " + alias : ""}`;
  return ret;
}
